...
def get_activity(code):
    ...
    if code in [101, 102, 199, 201, 202, 299, 301, 302, 399, 401, 402, 499, 502]:
        return "Work"
    elif code in [198, 298, 498, 598, 698, 798, 898, 998, 1098, 1198, 1298, 1398, 1498, 1598]:
        return "Travelling"
    elif code in [501]:
        return "Sell food"
...
# Organize data by ID (same person) and time (chronological order)
data = data.sort_values(["ID", "BEGIN"]) 
data = data.rename(columns={"ID": "sample_id", "BEGIN": "datatime", "Q401": "state"})

# Due to the size of the dataset (6000 people), we will only use a quarter of it for demonstration purposes
...
d3.movingbubbles(data, datetime="datatime", sample_id="sample_id", state="state", filepath="./moving_point.html",
                 note="Moving bubbles chart of how Vietnamese people spend their time in a day.", cmap="hsv",
                 center="Travelling", figsize=(780,800), size=2)